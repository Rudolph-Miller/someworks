...
(function() {
  var change_src, convert, img_box, imguploader, root, upload_box, upload_image, uploaded_tokens, uuids;
  ...
  convert = function() {
    var id, id1, id2, img, input, result, _i, _len;
    ...
    if (img) {
      return imguploader(input, result);
    } else {
      return $(".output-area").html(result);
    }
  };

  upload_image = function(id1, id2) {
    var def, file, files, formdata, url;
  ...
  change_src = function(url, id1, id2) {
    var input;
    input = $(".input-area").val();
    $(".input-area").val(input.replace("" + id1 + "/" + id2, url));
    $(".uploader").append(img_box(id1, id2));
    $("#form-" + id1 + "-" + id2).html(" ");
    return uuids.some(function(v, i) {
      if (v.id1 === id1 && v.id2 === id2) {
        return uuids.splice(i, 1);
      }
    });
  };

  imguploader = function(input, result) {
    var date, uuid1, uuid2;
    date = new Date();
  ...
  upload_box = function(uuid1, uuid2) {
    return "<form  id=\"form-" + uuid1 + "-" + uuid2 + "\" method=\"POST\" enctype=\"multipart/form-data\" action=\"writers/articles/upload_picture\"><input id=\"image-" + uuid1 + "-" + uuid2 + "\" type=\"file\" name=\"image\"></input></form>";
  };

  img_box = function(uuid1, uuid2) {
    return "<img id=\"result-" + uuid1 + "-" + uuid2 + "\" src=\"" + root + "/images/" + uuid1 + "/" + uuid2 + "\" style=\"max-height: 200px; height: auto;\"></img>";
  };
  ...
  $(function() {
    ...
    return $("#save").click(function() {
      return $("#tokens").val(uploaded_tokens);
    });
  });
